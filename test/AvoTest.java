...
public class AvoTest {
    class Avo extends Tavunu{
        Avo(){
            super();
        }
        
        Avo(String name, int birthYear, int pava) {
            super(name, 1, pava);
        }
    }
    @Test
    public void testCtorValidParams() {
        ...
        tv = new Avo("Trast", 1944, 0);
        assertEquals("Trast", tv.getName());
        assertEquals(10, tv.getPava());
        assertEquals(1944, tv.getBirthYear());

        tv = new Avo("Trast", 1944, 9);
        assertEquals("Trast", tv.getName());
        assertEquals(10, tv.getPava());
        assertEquals(1944, tv.getBirthYear());     
        
        tv = new Avo("Trast", 1944, 10);
        assertEquals("Trast", tv.getName());
        assertEquals(10, tv.getPava());
        assertEquals(1944, tv.getBirthYear());
        
        tv = new Avo("Trast", 1944, 11);
        assertEquals("Trast", tv.getName());
        assertEquals(11, tv.getPava());
        assertEquals(1944, tv.getBirthYear());
        
        tv = new Avo("Trast", 1944, 20);
        assertEquals("Trast", tv.getName());
    ...
    public void testToString() {
        var tv = new Avo("Trast", 1944, 12);
        
        assertEquals("Trast born in 1944 is an Avo with 12 pava.", tv.toString());
    }
